# Repositório da 7a fase - turma de Sistemas de Informação - Unibave / 2026

## Plano de ensino

### 25/02/2026 - [1a aula](https://github.com/Welquer/si-26-7a/tree/1a-aula)
    - Apresentação da disciplina
    - Alinhamento de conteúdo, conhecimento e expectativas com a turma
    - GitHub - novo repositório criado
    - Copiar projeto final do semestre anterior para sua branch no repositório atual e fazer executá-lo
    - Prática de versionamento de arquivos com Git
### 04/03/2026 - [2a aula](https://github.com/Welquer/si-26-7a/tree/2a-aula) - +0,5
    - Recaptulando: a internet
    - Estrutura cliente-servidor (e outras)
    - Protocolos de comunicação
    - Relação frontend e backend
    - Prática de frontend e comunicação com backend (API externa)
    - Prática de requisições assíncronas: Promises e async/await
    - Integração com DOM
### 11/03/2026 - [3a aula](https://github.com/Welquer/si-26-7a/tree/3a-aula) - +0,5
    - APIs: conceitos e tipos
    - Estado de aplicação na prática
        - no frontend com IndexedDB, Local e Session Storage
### 18/03/2026 - [4a aula](https://github.com/Welquer/si-26-7a/tree/4a-aula) - +0,5
    - API própria: expondo a aplicação
    - Protocolo HTTP e seus métodos
    - REST e RESTful
### 25/03/2026 - [5a aula](https://github.com/Welquer/si-26-7a/tree/5a-aula) - +0,5
    - GraphQL
### 01/04/2026 - [1a avaliação](https://github.com/Welquer/si-26-7a/tree/1a-avaliacao)
    - 1a avaliação
### 08/04/2026 - [6a aula](https://github.com/Welquer/si-26-7a/tree/6a-aula) - +0,5
    - Boas práticas de desenvolvimento:
        - reutilização de código
        - design patterns
        - TDD - Test Driven Design
        - DDD - Domain Driven Design
        - Autores referência
### 15/04/2026 - [7a aula](https://github.com/Welquer/si-26-7a/tree/7a-aula) - +0,5
    - TypeScript
      - Tuples
### 22/04/2026 - [8a aula](https://github.com/Welquer/si-26-7a/tree/8a-aula) - +0,5
    - Integração com banco de dados relacional
      - ACID
      - ORM
    - Banco de dados não relacional
### 29/04/2026 - [9a aula](https://github.com/Welquer/si-26-7a/tree/9a-aula) - +0,5
    - Segurança com cookies
        - HttpOnly
        - Secure
        - Path
    - CORS
    - SSL/TLS
    - CSP
### 06/05/2026 - [10a aula](https://github.com/Welquer/si-26-7a/tree/10a-aula) - +0,5
    - Aula prática para reforço de todo conteúdo visto
### 13/05/2026 - [11a aula](https://github.com/Welquer/si-26-7a/tree/11a-aula) - +0,5
    - Aula prática para reforço de todo conteúdo visto
### 20/05/2026 - [2a avaliação](https://github.com/Welquer/si-26-7a/tree/2a-avaliacao)
    - 2a avaliação
### 27/05/2026 - [12a aula](https://github.com/Welquer/si-26-7a/tree/12a-aula) - +0,5
    - Algoritmos de hash
    - Criptografia
    - Autenticação e autorização
        - sessão de usuário
        - tokens e refresh tokens
    - JWT
### 03/06/2026 - [13a aula](https://github.com/Welquer/si-26-7a/tree/13a-aula) - +0,5
    - Teste de software
        - unitário
        - interface
        - integração
    - CI/CD
    - POSIX
### 10/06/2026 - [14a aula](https://github.com/Welquer/si-26-7a/tree/14a-aula)
    - Arquitetura
    - Monolitos e microserviços
### 17/06/2026 - [15a aula](https://github.com/Welquer/si-26-7a/tree/15a-aula)
    - prática / trabalho final
### 24/06/2026 - [3a avaliação](https://github.com/Welquer/si-26-7a/tree/3a-avaliacao)
    - 3a avaliação
### 01/07/2026 - [N-1](https://github.com/Welquer/si-26-7a/tree/n-1)
    - N-1

____

### Observações
- Duas chamadas serão feitas por aula, uma no início e outra no final
- A cada aula deve ser feito pelo menos um commit até às 22h
- Nota 1 vale 4 pontos + 0,5 por aula que vale ponto (12 aulas) = 10 pontos
- Notas 2 e 3 valem 10 pontos

____

Para as disciplinas de quatro créditos: Serão direcionadas 18 horas para realização do autoestudo.
OBS.: Esse Plano de Ensino poderá sofrer alterações por determinação institucional, por motivos de caso fortuito ou força maior ou ainda por necessidades acadêmicas e procedimentais que se verificarem necessárias no decorrer do processo de ensino/aprendizagem, sem quaisquer ônus ou responsabilidades para a instituição e/ou prepostos.
